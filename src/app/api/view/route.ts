...
export async function POST(request: Request) {
  ...
  // onConflict обязан в точности совпадать с уникальным индексом
  // page_views_unique_visitor_day, иначе Postgres отвечает "нет
  // подходящего ограничения" и не пишет вообще ничего. Колонку day не
  // передаём: её ставит значение по умолчанию (current_date), тем же
  // временем сервера БД, что и границы периодов в site_view_counts —
  // если считать день на стороне Node, около полуночи они разъедутся.
