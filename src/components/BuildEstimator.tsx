...
function pricePerUnit(size: number) {
  return size >= 400 ? 1.5 : 1.25;
}

function estimatePrice(width: number, height: number) {
...
// far from a real quote, hence the ± range in estimateDeadlineDays below.
const DEADLINE_ANCHORS: [size: number, days: number][] = [
  [150, 7],
  [400, 14],
  [600, 24.5],
];

function estimateDeadlineDays(width: number, height: number) {
  const size = effectiveSize(width, height);
